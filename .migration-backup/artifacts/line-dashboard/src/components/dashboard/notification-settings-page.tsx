...
export function NotificationSettingsPage() {
  ...
  useEffect(() => {
    const s = loadNotificationSchedule()
    setTimes(s.times)
    setHydrated(true)
  }, [])

  const schedulePayload = useMemo((): NotificationScheduleV1 => {
    return {
      timezone: NOTIFICATION_TIMEZONE,
      times,
    }
  ...
  useEffect(() => {
    if (!hydrated) return
    saveNotificationSchedule(schedulePayload)
  }, [hydrated, schedulePayload])
              ...
              <div className="space-y-2">
                ...
                {savedFlash && (
                  <p className="text-center text-xs text-primary" role="status">
                    บันทึกแล้ว (localStorage · เวลาไทย)
                  </p>
                )}
              </div>
            </form>
